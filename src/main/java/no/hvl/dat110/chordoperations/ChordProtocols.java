...
public class ChordProtocols {
	...
	public void fixFingerTable() {
		 ...
		 try {
		        ...
		        List<NodeInterface> fingerTable = chordnode.getFingerTable();  // get the finger table from the chordnode
		        fingerTable.clear();  // clear the current finger table

		        BigInteger addressSize = Hash.addressSize();  // get the address size from the Hash class. This is the modulus and our address space (2^mbit = modulus)
		        int numBits = Hash.bitSize();  // get the number of bits from the Hash class. Number of bits = size of the finger table

		        for (int i = 0; i < numBits; i++) {
		            BigInteger k = chordnode.getNodeID().add(BigInteger.valueOf(2).pow(i)).mod(addressSize);  // compute: k = (n + 2^i) mod 2^mbit
		            NodeInterface succNode = chordnode.findSuccessor(k);  // find the successor of k

		            if (succNode != null) {  // check that succnode is not null
		                fingerTable.add(succNode);  // add it to the finger table
		            } else {
		                fingerTable.add(chordnode);  // if it's null, add the current node to the finger table
		            }
		        }

		       // chordnode.setFingerTable(fingerTable);  // update the finger table of the chordnode
